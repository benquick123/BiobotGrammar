...
def simulate(robot, task, opt_seed, task_args, neuron_stream=None):
    ...
    side_sim = EnvWrapper(make_sim_and_task_fn)
...
def simulate_old(robot, task, opt_seed, thread_count, episode_count=1):
    ...
    for episode_idx in range(episode_count):
        ...
        if episode_idx < episode_count - 1:
            ...
            # replay_returns = np.concatenate((replay_returns, returns[:task.episode_len]))
...
def main():
    ...
    if args.log_file:
        ...
        log_path = os.path.join(args.log_dir, f'mcts_{datetime.datetime.now():%Y%m%d_%H%M%S}.csv')
